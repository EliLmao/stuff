...
namespace MYREFORMEDASSIGNMENT
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            stopwatch.Stop();
            //Console.WriteLine("Elapsed time: {0} milliseconds", stopwatch.ElapsedMillisecondsx);
            // file writing
            FileStream outfile = new FileStream(args[1], FileMode.Create, FileAccess.Write);
            ...
            //double time = Convert.ToDouble(stopwatch.Elapsed);
            ...
            //Console.WriteLine(string.Format("{0:00.00}", Convert.ToDouble(stopwatch.Elapsed)));
            ...
            for (int i = 0; i < departtime.Count; i++)
            {
                TotalTime = +stationdurations[i];

                string timedepart = string.Format("{0:00}:{1:00}", hourtime, mintime);
                departtime.Add(timedepart);

                if (plane.range <= TotalTime)
                {
                    TotalTime = +plane.refueltime;
                    mintime = +plane.refueltime;
