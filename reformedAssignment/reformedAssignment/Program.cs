...
namespace Assignment
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //void readstation(string fileLocation)
            //{
            Properties readvar = new Properties();
            //Easy way to read
            string[] filelines = File.ReadAllLines(args[1]);

            //Other approach: file stream
            FileStream inFile = new FileStream(args[1], FileMode.Open, FileAccess.Read);
            StreamReader reader = new StreamReader(inFile);
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                readvar.LocationName = stationDetails[0];

                readvar.Xaxis = Convert.ToInt32(stationDetails[1]);

                readvar.Yaxis = Convert.ToInt32(stationDetails[2]);
                ...
                getStation a = new getStation(readvar.LocationName, readvar.Xaxis, readvar.Yaxis);

                //stationlist.Add(stationDetails[0]);
  
                //for (int i = 0; i < 30; i++)
                //{
                    Console.WriteLine("{0} {1} {2}", a.GetLocationName(), a.GetXaxis(), a.GetYaxis());
                //}
            ...
            //string hello = File.ReadAllText(args[1]);

            //Console.WriteLine(hello);

            //stations.readstation(args[1]);
